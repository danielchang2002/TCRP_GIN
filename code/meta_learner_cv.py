...
class MetaLearner(object):
    ...
    def zero_shot_test(
        self, unseen_train_loader, unseen_vali_loader, unseen_test_loader
    ):
    ...
    def unseen_tissue_learn(self, unseen_train_loader, unseen_test_loader):
        ...
        (
            ...
            test_prediction,
            test_true_label,
        ) = evaluate_new(unseen_tissue_model, unseen_test_loader, 0)

        return (
            mtrain_loss,
            mtrain_pear_corr,
            mtrain_spearman_corr,
            mtest_loss,
            mtest_pear_corr,
            mtest_spearman_corr,
            test_prediction,
    ...
    def train(self):
        ...
        train_spearman_corr, test_spearman_corr = np.zeros(
            (self.num_updates,)
        ), np.zeros((self.num_updates,))

        for epoch in range(self.num_updates):

            # Collect a meta batch update
            grads = []
            meta_train_loss, meta_train_corr, meta_val_loss, meta_val_corr = (
                np.zeros((self.meta_batch_size,)),
                np.zeros((self.meta_batch_size,)),
                np.zeros((self.meta_batch_size,)),
                np.zeros((self.meta_batch_size,)),
            )
            ...
            for i in range(self.meta_batch_size):
                ...
                observed_train_loader, observed_test_loader = get_observed_data_loader(
                    self.meta_dataset.feature,
                    self.meta_dataset.label,
                    self.tissue_index_list,
                    self.K,
            ...
            (
                train_loss[epoch],
                train_corr[epoch],
                train_spearman_corr[epoch],
                test_loss[epoch],
                test_corr[epoch],
                test_spearman_corr[epoch],
                _,
                _,
            ) = self.unseen_tissue_learn(unseen_train_loader, unseen_test_loader)
            ...
            print(
                ...
                epoch,
                "train_loss:",
                float("%.3f" % train_loss[epoch]),
                "train_pearson:",
                float("%.3f" % train_corr[epoch]),
                "train_spearman:",
                float("%.3f" % train_spearman_corr[epoch]),
                end=" ",
            )
            print(
                "test_loss:",
                float("%.3f" % test_loss[epoch]),
                "test_pearson:",
                float("%.3f" % test_corr[epoch]),
                "test_spearman:",
                float("%.3f" % test_spearman_corr[epoch]),
            )
        ...
        print(
            ...
            float("%.3f" % best_train_loss_test_corr),
            "best_train_corr_test_corr",
            float("%.3f" % best_train_corr_test_corr),
            "best_train_corr_test_scorr",
            float("%.3f" % best_train_corr_test_scorr),
            "best_train_scorr_test_corr",
