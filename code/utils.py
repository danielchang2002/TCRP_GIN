...
def load_data_cell_line(
    ...
    for temp_tissue, temp_cell_line_list in list(tissue_map.items()):
        ...
        if len(temp_cell_line_list) == 0:
            continue
        feature_map[temp_tissue + "_" + drug] = scaler.transform(
            feature_map[temp_tissue + "_" + drug]
        )
    ...
    print(
        ...
        len(cv_feature_list),
        len(cv_label_list),
        len(vali_feature_list),
        len(vali_label_list),
    )
    print(
        "Vali dimension",
        cv_feature_list[0].shape,
        vali_feature_list[0].shape,
        vali_label_list[0].shape,
    )
...
def load_data_PDTC(
    drug, path="/home-nfs/wangsheng/challenge_GDSC_PDTC/GDSC_PPI_feature/"
):
...
def load_data(
    ...
    drug,
    K,
    path="/home-nfs/wangsheng/challenge_GDSC_PDTC/GDSC_PPI_feature/",
):
    # Load dataset
    print("Loading feature and label files...")
